...
/****** Get Each Line in a File ******/
	...
	(
		$filepath		// <str> The full file path of the file to retrieve.
	)					// RETURNS <int:str> An array of the file's lines (in order).
	
	// $fileLines = File::getLines("/path/to/file/myfile.txt");
	{
		if(self::exists($filepath))
		{
...
/****** Append to a File ******/
# This method appends new content to a file. If the file doesn't exist, it creates it. The
# permissions need to be valid in order to write to this file.
# Note: this file will attempt to create the directories leading up to the file if they do not currently exist.
	...
	// File::append("/path/to/file/myfile.txt", "Some content that I would like to append to this file.");
	{
		// Pull the existing contents
		$fileContents = self::read($filepath);
		
		// Append the content to the file
		return File::write($filepath, $fileContents . $text);
	}
	
	
/****** Prepend text to a File ******/
# This method prepends new content to a file. If the file doesn't exist, it creates it. The
# permissions need to be valid in order to write to this file.
# Note: this file will attempt to create the directories leading up to the file if they do not currently exist.
	...
	// File::prepend("/path/to/file/myfile.txt", "Some content that I would like to append to this file.");
	{
		// Pull the existing contents
		$fileContents = self::read($filepath);
		
		// Append the content to the file
		return File::write($filepath, $text . $fileContents);
	}
...
# This method modifies a file's content with content of your own. The file must exist. Permissions must be valid.
	...
	(
		$filepath		// <str> The full file path of the file to edit.
	,	$replace		// <str> The text to include in the file.
	)					// RETURNS <bool> TRUE on success, FALSE on failure.
...
# This method injects a substring into a file. The file must exist. Permissions must be valid.
	...
	(
		$filepath			// <str> The full file path of the file to edit.
	,	$inject				// <str> The text to inject into the file.
	)						// RETURNS <bool> TRUE if updated properly, FALSE if something went wrong.
...
/****** Prune a File ******/
	...
	(
		$filepath		// <str> The full file path of the file to retrieve.
	,	$allowedLines	// <int> The number of lines to allow (prune the rest)
	)					// RETURNS <bool> TRUE on success, FALSE on failure.
	
	// File::prune("/path/to/file/myfile.txt", $allowedLines);
	{
		$fileText = self::read($filepath);
		
		$lines = substr_count($fileText, "\n");
		
		if($lines > $allowedLines)
		{
			$getLines = self::getLines($filepath);
			array_splice($getLines, 0, $allowedLines);
...
/****** Set Permissions of a File ******/
	...
	,	$permissionMode = 755	// <int> The number used to set the permission mode. (i.e. 0755, 755, etc)
	)							// RETURNS <bool> TRUE on success, FALSE on failure.
