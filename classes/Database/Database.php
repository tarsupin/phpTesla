...
This class provides an SQL wrapper that safely handles connections and queries with the database. For database commands that affect the structure of tables, refer to the "Database_Meta" class, which requires root access.

This plugin doesn't use ORM - all queries are made with vanilla SQL.
...
/****** Class Variables ******/
	public static $databaseName = "";		// <str> The name of the database
	public static $rowsAffected = 0;		// <int> The number of rows affected by an UPDATE or INSERT query
	public static $lastID = 0;				// <int> The last ID sent by an INSERT query
	
	public static $database = null;		// <mixed> The database resource
	public static $inTransaction = 0;		// <int> The depth of transaction that is currently being run
	public static $rootActive = false;		// <bool> TRUE if the admin DB user is being used
...
/****** Initialize the Database ******/
	...
	,	$asAdmin = false	// <bool> TRUE if you want to run as an admin
	...
	// Database::initialize($databaseName);
	{
		...
		try
		{
			...
			// Connect to the database as a standard user
			else
			{
		...
		catch (PDOException $e)
		{
			...
			if(!empty($url[0]) and $url[0] == "install") {
				throw new Exception($e);
				// "Haven't run /install yet, or might not have driver installed: apt-get install php-mysql"
			}
		}
...
/****** Initialize Root Access to the Database ******/
	...
	// Database::initRoot($databaseName);
	{
		if(self::$rootActive == true) { return true; }
		
		if(self::initialize($databaseName, true))
		{
...
/****** Select a Row from the Database ******/
	...
	(
		$query			// <str> The SQL for the selection query that you're going to run.
	,	$inputArray		// <int:mixed> The values that correspond to the PDO ?'s in the query.
	)					// RETURNS <str:mixed> an array of the row that was requested
...
/****** Select a Value from the Database ******/
	...
	(
		$query			// <str> The SQL for the selection query that you're going to run
	,	$inputArray		// <int:mixed> The values that correspond to the PDO ?'s in the query
	)					// RETURNS <mixed> the value of the first column
...
/****** Select Multiple Rows from the Database ******/
	...
	(
		$query			// <str> The selection query to run (must start with "SELECT")
	,	$inputArray		// <int:mixed> The values that correspond to the ?'s in the query
	)					// RETURNS <mixed> <int:[str:mixed]> array of arrays that contain each row you retrieved
...
/****** End a Transaction ******/
	...
	(
		$commit = true		// <bool> Set to FALSE if the transaction should roll back instead of committing.
	)						// RETURNS <bool> TRUE on success, FALSE on failure
...
# Returns a TUPLE that provides the "IN" WHERE statement for the filter, and the SQL ARRAY contents for it.
	...
	(
		$sqlFilters 	// <str:[int:mixed]> The list of SQL Filters: array("column" => array($val, $val), ...)
	)					// RETURNS <int:mixed> the query string and array of the filter.
