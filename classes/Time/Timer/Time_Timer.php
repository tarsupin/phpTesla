...
-----------------------------------------------------
------ Simple examples of setting a Time_Timer ------
-----------------------------------------------------
...
You can also set a timer when initializing it, like this:
	...
	$timer = new Time_Timer($currentTime, $inOneHour, 30);

	
----------------------------------
------ Accessing Time_Timer Data ------
----------------------------------
...
This class has several methods to allow you to do this, as well as allows you to save timers and reload them later.
	...
	// Save a timer
	$timer = new Time_Timer(time(), 3600, 25);
	$saveData = $timer->save();
	
	// Load a timer
	$timer = new Time_Timer($saveData);
...
an internal pointer. The Time_Timer class uses the $timer->timerActivation variable to track when you last attempted to
...
------------------------------------------------------------
------ Fully Functional Cross-Page Time_Timer Example ------
------------------------------------------------------------
	
If you load this code and refresh the page every few seconds, you'll notice how it goes from saying "Time_Timer activated!"
to "You have X more seconds until this timer activates..."
	...
	else
	{
		...
		$timer = new Time_Timer($_SESSION['saveTimer']);
	}
	
	// Attempt to trigger the timer
	if($timer->trigger())
	{
		echo "Time_Timer activated!";
	}
	else
	{
...
$timer = new Time_Timer($saveData)						// Load a timer from saved data
$timer = new Time_Timer($tickInterval)					// Sets never-expiring timer with a specified tick interval
$timer = new Time_Timer($start, $end, $tickInterval)		// Sets timer with start, end, and a specified tick interval
...
$timer
	->start($time)						// Sets the start time with a timestamp or a Time_Tracker object
	->end($time)						// Sets the end time with a timestamp or a Time_Tracker object
...
/****** Class Variables ******/
	...
	public $timerStart = 0;			// <int> The timestamp that the timer starts at.
	public $timerEnd = 0;			// <int> The timestamp that the timer ends at; default is never ends.
	public $timerActivation = 0;	// <int> The timestamp of the last activation.
	public $tickInterval = 0;		// <int> The number of seconds between timer ticks / triggers.
...
/****** Save a timer ******/
	...
	)				// RETURNS <str> JSON-minified string that can be used to restore a timer.
...
/****** Load a schedule ******/
	...
	(
		$saveData	// <str> A serialized value (minimized json) to restore a schedule.
	)				// RETURNS <void>
...
/****** Specify the timer's starting time ******/
	...
	(
		$unitData	// <mixed> The timestamp or time tracker array to start at.
	)				// RETURNS <this>
...
/****** Specify the timer's ending time with a duration rather than coded timestamp ******/
	...
	(
		$seconds	// <int> The number of seconds to run the timer.
	)				// RETURNS <this>
...
/****** Set the frequency of events for the timers (in days) ******/
	...
	(
		$days		// <int> The number of days to cycle.
	)				// RETURNS <this>
...
/****** Set the frequency of events for the timers (in hours) ******/
	...
	(
		$hours		// <int> The number of hours to cycle.
	)				// RETURNS <this>
...
/****** Set the frequency of events for the timers (in minutes) ******/
	...
	(
		$minutes	// <int> The number of minutes to cycle.
	)				// RETURNS <this>
...
/****** Set the frequency of events for the timers (in seconds) ******/
	...
	(
		$seconds	// <int> The number of seconds to cycle.
	)				// RETURNS <this>
...
/****** Attempt to trigger the timer ******/
	...
	)				// RETURNS <bool> TRUE if the timer was activated, FALSE if not.
...
/****** Get the first available divisible timestamp of this timer ******/
	...
	)					// RETURNS <int> first available divisible timestamp.
...
/****** Get the timestamp of the most recent activation ******/
	...
	)					// RETURNS <int> timestamp of the previous event.
...
/****** Get the timestamp of the next event activation ******/
	...
	)					// RETURNS <int> timestamp of the next event.
...
/****** Get an list of upcoming events ******/
	...
	(
		$limit = 100	// <int> The maximum number of events to return.
	)					// RETURNS <int:int> of timestamps, ordered by next to arrive.
